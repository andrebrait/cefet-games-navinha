...
public class Asteroid implements Entity, Collidable {
	...
	final void recycle(float minimumY) {
		position.set(MathUtils.random(area.width), MathUtils.random(minimumY, area.height));
	...
	public boolean collidesWith(Collidable other) {
		// Asteroid vs LaserShot: rect vs rect && rect vs circle
		...
		if (other instanceof LaserShot) {
			return Collision.rectsOverlap(bounds, other.getMinimumBoundingRectangle())
					&& Collision.circleRectOverlap(circle, other.getMinimumBoundingRectangle());
		} else if (other instanceof Asteroid || other instanceof Ship || other instanceof VortexShot) {
